# GPE.UI — PHASE 0
# MODULAR GAME UI SYSTEM — RESEARCH / ARCHITECTURE / MIGRATION MASTER MISSION v0.2
...
Cette version `v0.2` remplace la `v0.1`.

Elle intègre la revue adversariale du master prompt v0.1.

La correction méthodologique centrale est la suivante :
...
Le but de cette Phase 0 n'est donc PAS :
...
Il est préférable d'identifier correctement 4 incertitudes structurantes que de produire 20 rapports superficiels couvrant toutes les technologies UI connues.
...
Les principes suivants sont des hypothèses de travail.
...
L'UI devrait parler en intentions telles que :
...
plutôt qu'être câblée directement à des touches physiques.
...
Seulement les limites démontrées par le code ou des consumers réels.
...
Avant de parler d'architecture, produire la liste des problèmes structurels réellement observés.
...
Identifier les **5 incertitudes à plus fort impact**.
...
Ne présume pas que ce sont les bons cinq.

## Gate 2 PASS

Le programme de recherche restant doit être explicitement réduit à ce qui est nécessaire pour ces incertitudes.
...
Si des sibling repositories sont présents localement :
...
Pour chaque architecture candidate, définir explicitement l'ordre temporel d'une frame UI.
...
Comparer les variantes nécessaires.
...
Cette question est structurante et prioritaire.
...
et au moins une langue hors latin comme pression conceptuelle.
...
Pour chaque architecture candidate :
...
```text
19 fichiers superficiels créés juste pour satisfaire une checklist
```
...
Avec information-gain strategy.
...
```text
Top 10 ways this architecture could fail
```
