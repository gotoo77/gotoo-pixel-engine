...
mod tests {
    ...
    fn ball_starts_attached_and_launch_releases_it() {
        ...
        assert!(game.ball_stuck);
        let initial_x = game.ball_x;

        game.paddle_x += 20.0;
        game.attach_ball_to_paddle();
        assert!(game.ball_x > initial_x);

        assert!(game.launch_ball());
        assert!(!game.ball_stuck);
        assert!(!game.launch_ball());
    }

    #[test]
    fn touch_mode_has_three_virtual_actions() {
        let game = BreakoutGame::new_touch();
        assert_eq!(
    ...
    fn level_progression_increases_speed_and_relaunches() {
        let mut game = BreakoutGame::new();
        let first_speed = game.ball_vx.abs();
        game.advance_level();
        assert_eq!(game.level, 2);
        assert!(game.ball_vx.abs() > first_speed);
        assert!(game.ball_stuck);
        assert!(game.bricks.iter().all(|brick| brick.active));
    }
    ...
    fn substeps_catch_fast_brick_collision() {
        ...
        let brick = game.bricks[brick_index];
        game.ball_stuck = false;
        game.ball_x = brick.rect.x as f32 + 4.0;
        game.ball_y = brick.rect.y as f32 + brick.rect.height as f32 + 8.0;
        game.ball_vx = 0.0;
        game.ball_vy = -900.0;

        let feedback = game.update_ball(0.02);

        assert!(!game.bricks[brick_index].active);
        assert!(game.score > 0);
    ...
    fn sound_bank_owns_breakout_feedback_assets() {
        ...
        assert!(game.sounds.contains(LAUNCH_SOUND));
        assert!(game.sounds.contains(PADDLE_HIT_SOUND));
        assert!(game.sounds.contains(BRICK_HIT_SOUND));
        assert!(game.sounds.contains(LIFE_LOST_SOUND));
        assert!(game.sounds.contains(LEVEL_CLEAR_SOUND));
        assert!(game.sounds.contains(GAME_OVER_SOUND));
    }
}
