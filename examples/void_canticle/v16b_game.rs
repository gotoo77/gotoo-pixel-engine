...
impl VoidCanticleV16B {
    ...
    fn render_canticle_ready_feedback(&self, framebuffer: &mut Framebuffer) {
        if self.base().core_charge < CORE_MAX {
            return;
        }

        let pulse = ((self.base().animation_time * 9.0) as i32 & 1) == 0;
        let color = if self.canticle_ready_flash_timer > 0.0 || pulse {
            CANTICLE_COLOR
        } else {
            CINDER
        };
        framebuffer.draw_rect(32, 277, 142, 11, color);
        framebuffer.draw_text(112, 269, "READY", color);
    }
    ...
    fn render_void_reveal(&self, framebuffer: &mut Framebuffer) {
        ...
        draw_centered_text(framebuffer, 5, headline, VOID_LIGHT);
        draw_centered_text(
            framebuffer,
            16,
            consequence,
    ...
    fn render_boss_phase_banner(&self, framebuffer: &mut Framebuffer) {
        ...
        let text = format!("BELLKEEPER / {}", bell_phase_name(phase));
        draw_centered_text(framebuffer, 7, &text, CANTICLE_COLOR);
    }
    ...
    fn render_polish(&self, framebuffer: &mut Framebuffer) {
        ...
        self.render_canticle_ready_feedback(framebuffer);
