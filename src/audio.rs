...
mod tests {
    ...
    #[test]
    fn decodes_runtime_owned_wav_buffer() {
        let bytes = VALID_WAV.to_vec();
        let sound = decode_wav(&bytes).expect("owned runtime buffer should decode");
        assert_eq!(sound.samples.len(), 2);
    }
    #[test]
    ...
    #[test]
    fn noop_audio_registers_runtime_owned_wav() {
        let mut audio = NoopAudio::default();
        let bytes = VALID_WAV.to_vec();
        audio
            .register_wav(TEST_SOUND, &bytes)
            .expect("runtime buffer should register");
        drop(bytes);
